...
func (h *PsyncHandler) processCmd(parsedResult []resp.ParsedCmd) []string {
	...
	ackCmd := resp.HandleEncode(respEncodingConstants.STRING, fmt.Sprintf("%s %s %s", CMD_RESPONSE_FULL_RESYNC, h.replicationStore.MasterReplId, h.replicationStore.Offset))

	encodingCmd := resp.HandleEncode(respEncodingConstants.BULK_STRING, string(decoded))
